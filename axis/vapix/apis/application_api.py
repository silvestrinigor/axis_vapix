...
class RequestAplicationApi(IRequestAxisVapix):
    ...
    def set_configure_application(self, config_name:str, config_value: bool): # TODO: Test if this function works
        ...
        """
        request = Request("POST", f"http://{self._host}:{self._port}/{ApiPathType.AXIS_CGI_APPLICATIONS_CONFIG.value}?{RequestUrlParamType.ACTION.value}={ActionType.SET.value}&{config_name}={config_value}")
        return request
    ...
    def get_configure_application(self, config_name: str): # TODO: Test if this function works
        ...
        :param config_name: String.
        """
        request = Request("POST", f"http://{self._host}:{self._port}/{ApiPathType.AXIS_CGI_APPLICATIONS_CONFIG.value}?{RequestUrlParamType.ACTION.value}={ActionType.GET.value}&{config_name}")
        return request
    
    def list(self): # TODO: Test if this function works
        request = Request("POST", f"http://{self._host}:{self._port}/{ApiPathType.AXIS_CGI_APPLICATIONS_LIST.value}")
        return request
