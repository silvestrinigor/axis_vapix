...
class Device:
    ...
    def serial_number(self):
        ...
        response = axis_methods.get_properties(axis_request=self._request_maker, properties=properties)
    ...
    def date_time(self):
        ...
        response = axis_methods.get_date_time_info(self._request_maker)
        return handlers.DateTimeInfoResponseHandler(response=response).date_time

    @date_time.setter
    def date_time(self, date_time: datetime):
        self._request_maker.api_version = handlers.ApisInfoResponseHandler(self._apis_list_response).get_supported_api_version(types.ApiType.AXIS_CGI_TIME)
        response = axis_methods.set_date_time(self._request_maker, date_time=date_time)
        if response.status_code != 200 or handlers.is_response_with_error(response=response) == True:
            raise Exception()
        return
    ...
    def time_zone(self, time_zone: types.TimeZoneType):
        response = axis_methods.set_time_zone(self._request_maker, time_zone=time_zone)
        if response.status_code != 200 or handlers.is_response_with_error(response=response) == True:
            raise Exception()
        return
    ...
    def set_posix_time_zone(self, posix_time_zone, enable_dst: bool):
        self._request_maker.api_version = handlers.ApisInfoResponseHandler(self._apis_list_response).get_supported_api_version(types.ApiType.AXIS_CGI_TIME)
        response = axis_methods.set_posix_time_zone(self._request_maker, posix_time_zone=posix_time_zone, enable_dst=enable_dst)
        if response.status_code != 200 or handlers.is_response_with_error(response=response) == True:
            raise Exception()
        return
    
    def is_this_api_supported(self, api_type: types.ApiType):
        response = axis_methods.get_api_list(self._request_maker)
        if response.status_code != 200 or handlers.is_response_with_error(response=response) == True:
            raise Exception()
    ...
    def _update_apis_json_info(self):
        ...
        self._apis_list_response =  axis_methods.get_api_list(self._request_maker)
